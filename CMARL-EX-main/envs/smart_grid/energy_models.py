...
class Carbon:
    def __init__(self, carbon_file="carbon_intensity.csv", hourly_timesteps=4):
        self.file = carbon_file
        self.hourly_timesteps = hourly_timesteps
        self.weather_data = pd.read_csv(carbon_file)
        self.data = self.preprocess_data()

    def preprocess_data(self):
        mapping_dict = {'kg_CO2': 'kg_CO2'}
        res = {}
        for k, v in mapping_dict.items():
            if k in ['kg_CO2']:
                res[k] = subhourly_lin_interp(self.weather_data[v], self.hourly_timesteps)
        return res
class Building:
    def __init__(self, data_path, climate_zone, buildings_states_actions_file, hourly_timesteps, uid, weather,
        ...
        self.buildings_net_electricity_demand =0
        self.phi = 0
        self.year = 0
        self.rbc = False
    ...
    def get_carbonreward(self): # dummy cost function
        if (self.current_gross_electricity_demand + self.current_gross_generation)>0:
            pwr = ((self.current_gross_electricity_demand + self.current_gross_generation) / 50) ** 4
            pwr1 = (self.current_gross_electricity_demand + self.current_gross_generation)
        else:
            pwr = 0
            pwr1 = 0

        with open("carbon_intensity.csv") as csv_file:
            weather_data = pd.read_csv(csv_file)

        mapping_dict = {'kg_CO2':'kg_CO2'}
        res = {}
        for k,v in mapping_dict.items():
            if k in ['kg_CO2']:#self.enabled_states[k]:
                res[k] = subhourly_lin_interp(weather_data[v], self.hourly_timesteps)

        car_raw = pwr1 * res['kg_CO2'][self.time_step]
        ca_r = ((pwr1 * res['kg_CO2'][self.time_step])/40)**4
        return ca_r,car_raw,pwr,pwr1
    ...
    def get_devs(self, net): # dummy cost function
        ...
        if abs(dev)<=0.04:
            ...
            reward = 2000 * (abs(dev) - 0.04)**2
        else:
            reward = 0.2+ 500 * (abs(dev) - 0.05)
        return reward
    ...
    def get_demandreward(self, net):

        if (self.current_gross_electricity_demand + self.current_gross_generation)>0:
            pwr = (self.current_gross_electricity_demand + self.current_gross_generation)
        else:
            pwr = 0
        return pwr

    def get_demandreward_power(self, net):
    ...
    def step(self, a):
        ...
        self.buildings_net_electricity_demand = -1*round(self.current_gross_electricity_demand + self.current_gross_generation, 4)
