...
class RankingLimitDataset(data_utils.Dataset):
    def __init__(self, traj_files, pair_nums, state_dim, action_dim, mode='state_only', traj_len=50, seed=1234):
        ...
        for i in range(len(traj_files)):
            loaded_data = torch.load(open(traj_files[i], 'rb'))
    ...
    def get_all_pairs(self, trajs, rewards, traj_len):
        all_pairs = []
        print(len(trajs),len(rewards))
        for i in range(1):
            if traj_len > 0:
                for j in range(max(1, len(rewards)-traj_len*self.jump_steps+1)):
                    all_pairs.append([j,0,np.sum(rewards[j:j+traj_len*self.jump_steps:self.jump_steps],axis=0)])
            else:
